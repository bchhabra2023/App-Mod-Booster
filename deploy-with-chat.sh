...
DEPLOYMENT_NAME="expense-modernize-chat"
LOCATION="uksouth"
...
echo "[2/14] Deploying main.bicep (deployGenAI=true)..."
...
print(outputs['openAIEndpoint']['value'])
print(outputs['openAIModelName']['value'])
print(outputs['searchEndpoint']['value'])
PY
)
...
OPENAI_ENDPOINT="${DEPLOY_VALUES[4]}"
OPENAI_MODEL_NAME="${DEPLOY_VALUES[5]}"
SEARCH_ENDPOINT="${DEPLOY_VALUES[6]}"
